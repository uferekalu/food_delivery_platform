...
/** Store counterpart of the restaurant sales-report page's `DownloadCsvButton` — see that
 * component's doc comment for why this fetches the CSV manually instead of a plain `<a href>`. */
function DownloadCsvButton({ storeId, from, to }: { storeId: string; from: string; to: string }) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      const res = await fetch(`/api/orders/store/${storeId}/sales-report/export${qs ? `?${qs}` : ""}`, {
      ...
      a.href = url;
      a.download = `sales-report-${storeId}.csv`;
...
function SalesReportView({ store }: { store: Store }) {
  const t = useTranslations("SalesReportPage");
  const tStore = useTranslations("StoreSalesReportPage");
  const locale = useLocale();
  const [from, setFrom] = useState("");
  const [to, setTo] = useState("");
  const { data, isLoading, isFetching, isError } = useGetStoreSalesReportQuery({
    storeId: store._id,
    from: from || undefined,
    to: to || undefined,
  });
  ...
  return (
    <Container className="flex flex-col gap-6 py-10">
      ...
      <Card>
        <CardContent className="flex flex-wrap items-end gap-4">
          ...
          <div className="ml-auto">
            <DownloadCsvButton storeId={store._id} from={from} to={to} />
          </div>
        </CardContent>
      </Card>
      ...
      {isLoading ? (
        ...
        <EmptyState title={t("noDeliveredOrdersInRange")} description={tStore("salesFiguresAppearHere")} />
      ) : (
        <>
          {data.itemsMissingCostPrice.length > 0 && (
            <Alert variant="warning" title={t("someItemsMissingCostPrice")}>
              {tStore("profitFiguresExclude", { items: data.itemsMissingCostPrice.join(", ") })}
            </Alert>
          )}
      ...
      <VendorOrderTransactionsSection sellerType="store" sellerId={store._id} from={from} to={to} />
...
function SalesReportPage({ id }: { id: string }) {
  ...
  return <SalesReportView store={store} />;
}

export default function DashboardStoreSalesReportPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return (
