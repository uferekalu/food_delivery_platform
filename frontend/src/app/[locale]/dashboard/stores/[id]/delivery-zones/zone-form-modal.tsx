...
export interface StoreZoneFormModalProps {
  open: boolean;
  onClose: () => void;
  storeId: string;
  currency: string;
...
export function StoreZoneFormModal({ open, onClose, storeId, currency, editing }: StoreZoneFormModalProps) {
  ...
  const [createZone, { isLoading: isCreating }] = useCreateStoreDeliveryZoneMutation();
  const [updateZone, { isLoading: isUpdating }] = useUpdateStoreDeliveryZoneMutation();
  ...
  async function submit(values: FormValues) {
    try {
      if (editing) {
        await updateZone({ storeId, zoneId: editing._id, body: values }).unwrap();
        toast({ title: t("zoneUpdated"), variant: "success" });
      } else {
        await createZone({ storeId, body: values }).unwrap();
