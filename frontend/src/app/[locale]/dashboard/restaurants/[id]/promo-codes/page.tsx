...
function CreatePromoForm({ restaurantId }: { restaurantId: string }) {
...
function PromoCodesManager({ restaurantId }: { restaurantId: string }) {
  const t = useTranslations("VendorPromoCodesPage");
  ...
  const promos = allMine?.filter((p) => p.restaurantId === restaurantId) ?? [];

  if (loadingRestaurant || loadingPromos) return <Skeleton className="h-64 w-full" />;
  if (!restaurant) return <Alert variant="danger">{t("restaurantNotFound")}</Alert>;

  return (
    <div className="flex flex-col gap-6">
      <p className="text-text-muted">{t("description")}</p>
      <CreatePromoForm restaurantId={restaurantId} />
...
export default function RestaurantPromoCodesPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const t = useTranslations("VendorPromoCodesPage");
  return (
    <RequireRole roles={["restaurant_owner", "admin"]}>
      <Container className="max-w-3xl py-10">
        ...
        <PromoCodesManager restaurantId={id} />
      </Container>
    </RequireRole>
  );
}
