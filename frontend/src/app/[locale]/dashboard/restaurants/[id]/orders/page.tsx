...
function OrderActions({ order }: { order: Order }) {
  ...
  // READY_FOR_PICKUP — the restaurant now picks who delivers it themselves (docs/ROADMAP.md
  // FDP-133), rather than only being able to wait for a rider to self-claim or for auto-dispatch.
  if (order.status === "READY_FOR_PICKUP") {
    return <AssignRiderAction order={order} />;
  }

  // ASSIGNED_TO_RIDER / PICKED_UP / OUT_FOR_DELIVERY — a rider is attached; the restaurant stays
  // able to see and contact them (and reassign, while still ASSIGNED_TO_RIDER) instead of the
  // order just vanishing from the queue (docs/ROADMAP.md FDP-133).
...
function OrderQueue({ restaurantId, restaurantName }: { restaurantId: string; restaurantName: string }) {
  ...
  useEffect(() => {
    ...
    const subscribe = () => socket.emit("restaurant:subscribe", { restaurantId });
    subscribe();
