...
function OAuthCallbackContent() {
  ...
  useEffect(() => {
    ...
    exchangeCode({ code })
      .unwrap()
      .then(() => router.replace("/"))
      .catch((err: unknown) => setExchangeError(getErrorMessage(err, "Couldn't complete sign-in")));
  }, [code, exchangeCode, router]);

  const error = code ? exchangeError : "Missing sign-in code — go back and try again.";
